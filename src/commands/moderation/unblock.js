...
module.exports = {
    ...
    .addUserOption(option => option.setName('target').setDescription('User to unblock.').setRequired(true))
    .addChannelOption(option => option.setName('channel').setDescription('Channel to unblock the user from.').addChannelTypes(ChannelType.GuildText))
    .addStringOption(option => option.setName('reason').setDescription('The unblock reason.').setMaxLength(1000).setMinLength(1)),
    /**
    ...
    async execute(interaction, client) {
        ...
        const CannotDoActionEmbed = new EmbedBuilder().setColor('Red').setDescription(`${Emojis.Error_Emoji} I cannot unblock this user OR they aren't blocked.`)
        ...
        interaction.reply({ embeds: [UnblockSuccessEmbed] });
    },
};